...
async function objectSolver() {
    // 处理资源配置映射表
    for (i = 0; i < assets_object["js"].length; i++) {
        var object = assets_object["js"][i];
    ...
    for (i = 0; i < assets_object["css"].length; i++) {
        var object = assets_object["css"][i];
        if (assets_map[object[0]] != undefined && assets_map[object[0]] != null) {
            alert("エラー", "アセットIDが重複している\nアセット設定ファイルをご確認ください", "リトライ");
            throw new Error("アセットIDが重複している: " + object[0]);
        }
    ...
    for (i = 0; i < assets_object["assets"].length; i++) {
